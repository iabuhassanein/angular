...
var CompaniesService = (function () {
    function CompaniesService(_http, _authService) {
        this._http = _http;
        this._authService = _authService;
        this._base_url = "http://kfupmf.mmed.email/backend/public/api/";
        this._base_url2 = "http://kfupm.mmed.email:8080/";
    }
    CompaniesService.prototype.getCompanies = function (offset, limit) {
    ...
    CompaniesService.prototype.getCompaniesApproved = function (offset, limit) {
        var params = this._authService.getToken("ROLE_TRAINING_DEPARTMENT");
        return this._http.get(this._base_url + 'trainingDepartment/company?approved=true&token=' + params + '&offset=' + offset + '&limit=' + limit)
            .map(function (res) { return res.json(); })
            .retry(2);
    };
    CompaniesService.prototype.approveCompany = function (id, index) {
        var params = this._authService.getToken("ROLE_TRAINING_DEPARTMENT");
        var headers = new http_1.Headers();
        var body = JSON.stringify({ "approved": 1 });
