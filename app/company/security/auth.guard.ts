...
export class AuthGuard implements CanActivate {
    constructor(private _router: Router, private _alertService: AlertService, private _authService: AuthenticationService) { }

    canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> | Promise<boolean> | boolean {
        if (this._authService.check('currentUser')) {
            try {
                // var obj = JSON.parse(this._authService.get('currentUser'));
                // if(obj.role == "ROLE_COMPANY_COORDINATOR") return true;
                
                let session: Session = this._authService.getSession();
                if(session.role == "ROLE_COMPANY_COORDINATOR") return true;
